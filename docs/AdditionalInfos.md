...
各アイテムに最後に触れた時期がわかります。

こちらの情報は、カードと同じと扱います。つまり、シンフォニーが全体公開を選ぶ場合、この情報は GM から全員へ伝えられます。

| キャラクター | カード種 |        カード名        |                                                                結果                                                                 |
| :----------: | :------: | :--------------------: | :---------------------------------------------------------------------------------------------------------------------------------: |
| シンフォニー | 持ち物 A |          灰皿          |                                                        シンフォニー（最近）                                                         |
| シンフォニー | 持ち物 B |  サイレンサー付き拳銃  |                                                        シンフォニー（最近）                                                         |
|  セレナーデ  | 持ち物 A |          メモ          |                                             アリア（1 年ほどまえ）、セレナーデ（最近）                                              |
|  セレナーデ  | 持ち物 B |         ナイフ         |                                              ララバイ（数か月前）、セレナーデ（最近）                                               |
|  セレナーデ  |  切り札  |         腕時計         |                                              アリア（1 年ほど前）、セレナーデ（最近）                                               |
|   ララバイ   | 持ち物 A |        変装道具        |                 ララバイ（数か月前）、セレナーデ（最近）。</br>中に入っている指輪はカプリッチオにぴったりのサイズだ                 |
|   ララバイ   | 持ち物 B |      起爆スイッチ      |        フーガ（最近）、ララバイ（最近）、セレナーデ（最近）。</br>解除するにはフーガかララバイの指紋が必要。生死は問わない。        |
|   キャロル   | 持ち物 A | フーガのスマートフォン |                        フーガ（最近）、キャロル（最近）。</br>フーガのスマートフォンのパスコードを入手する。                        |
|   キャロル   | 持ち物 B |        黄色の印        |                                   キャロル（最近）。</br>プレイヤーは追加ハンドアウトを入手する。                                   |
| カプリッチオ | 持ち物 A |           鍵           |                                                フーガ（最近）、カプリッチオ（最近）                                                 |
| カプリッチオ | 持ち物 B |          手錠          |                                                        カプリッチオ（最近）                                                         |
| カプリッチオ |  切り札  |   ロンドの研究ノート   |                                        ロンド（最近）、フーガ（最近）、カプリッチオ（最近）                                         |
| フーガの部屋 |          |      フーガの死体      |                   シンフォニー（最近）、セレナーデ（最近）、キャロル（最近）。</br>調べてみると石像が見つかった。                   |
| フーガの部屋 |          | アリアのスマートフォン | セレナーデ（１年以上前）、アリア（１年ほど前）、ララバイ（1 年ほど前）、フーガ（最近）。</br>パスコードを読み取ることは難しそうだ。 |
| フーガの部屋 |          |          金庫          |                                                  フーガ（最近）、キャロル（最近）                                                   |
| フーガの部屋 |          | 石板〈ハスターの招来〉 |                                                ロンド（1 年以上前）、フーガ（最近）                                                 |
...
セレナーデは腕時計（エルダーサイン）を最初から所有しているため、条件を満たしても黄色の印の HO を渡されない。その場合、黄色の印に執着していないため、他プレイヤーへ譲渡が可能である。この場合は、キャロルから他のキャラクターへ譲渡することが可能である。（譲渡されたカードは再度譲渡可能）

上記処理が混乱を生むと GM が判断した場合、エルダーサインは黄色の印に対する対抗手段となりえないと処理しても良い。
...
条件: カード「フーガのメモ帳」「ファロス灯台の歴史」「フーガの死体」のすべてを所有するプレイヤーが、呪文を使用したい旨を GM に申告する。キャロルの場合、「嫌悪感から目を背けた」と描写される。
...
やること: カード「鍵」を石板で上書きし、PL に「石板」を再確認させる（所有者も閲覧者も両方閲覧可能）。公開・非公開は鍵と同様にする。

2022/05/22 以降のバージョンのココフォリア盤面を使用している場合、カード「鍵」の削除で石板のカードが出現する。
